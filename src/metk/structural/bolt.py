...
from metk.structural.structural_object import StructuralObject, StructuralObjectGroup
...
class Bolt(StructuralObject):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if number:
            self.d = float(
                thread_data.query(f"`Size, Number or Inches` == {number.lstrip('#')}")[
        ...
        self.d = nearest_to(
            self.d, thread_data["Basic Major Diameter (in)"].infer_objects().to_list()
        )

        # Get the TPI for the given bolt size
        self.tpi = int(
            thread_data.query(f"`Basic Major Diameter (in)` == {self.d}")["TPI"]
        )
        ...
        if self.F_nt:
            ...
            if abs(self.Svx) > 1 or abs(self.Svy) > 1:
                ...
                return (
                    min(
                        1.3 * F_nt
                        - omega * F_nt / self.F_nv * (self.Svx**2 + self.Svy**2) ** 0.5,
                        F_nt,
                    )
                    / omega
                )
